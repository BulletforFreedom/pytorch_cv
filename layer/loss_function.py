...
class YOLO3Loss(nn.Module):
    ...
    def forward(self, prediction, labels, bboxes):
        ...
        [((img_size // stride) * pow(2,i)) for i in range(num_feature_map)]
        ...
        mask, noobj_mask, tx, ty, tw, th, weight_w_h, tconf, tcls = \
        self._get_target(labels, bboxes, total_anchors,
                         bs, img_size, num_feature_map, 
                         feature_map_size_list)
        ...
        loss = loss_x + loss_y + loss_w + loss_h + loss_obj_conf + loss_noobj_conf + loss_cls
