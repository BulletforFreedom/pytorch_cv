...
class Configer(object):
    def __init__(self, cfgfile):
        self.cfgfile=os.path.join(os.path.abspath('.')+'/config_file/'+cfgfile)
    ...
    def get_data_path(self):
        return os.path.join(self.net_info['current_path'],'data')
    ...
    def get_ckpt(self):
        ckpt_dir=os.path.join(self.get_backup_path()+'/'+self.net_info['ckpt'])
        if not os.path.isfile(ckpt_dir):
            print("{} is not found".format(ckpt_dir))
            sys.exit()
        print(">>> loading ckpt from '{}'".format(ckpt_dir))
        return ckpt_dir
