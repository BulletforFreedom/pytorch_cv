...
def run_get_map(dataset,model_name,test_info):
    ...
    for name in img_names:
        ...
        #if not detect box, jump next--guanyonglai
        if(len(gt_boxes.shape) != 2):
            continue
        #
        img_mAP, _, _, _ = my_eval.compute_ap(gt_boxes,
                                              gt_class_ids,
                                              pred_boxes,
                                              pred_class_ids,
                                              pred_scores)
        ...
        for cls in range(len(classes)):
            if cls in gt_class_ids:
                ...
                cls_AP, _, _, _ = my_eval.compute_ap(gt_b,
                                                     gt_c_ids,
                                                     pt_b,
                                                     pt_c_ids,
                                                     pt_s
    ...
    for i in range(len(cls_img_count)):
        if cls_img_count[i]==0.:
            cls_img_count[i]=1.
            AP[i]=1.
    AP = AP/cls_img_count
    mAP = sum(AP)/len(AP)
...
if __name__ == "__main__":
    ...
    ds_test = 'missfresh-yolo-voc-800-0705'
    ...
    for ds in DataSets:
        ...
        dataset_name = ds[1]
        test_info = ds[3]
        #dataset_name = sets_default = ['train','val']
        #model_name = (prefix,itr)   prefix_default = 'yolo-voc_'  itr in iterations_default = [25000]
#        test_info = 'yolo-voc'
